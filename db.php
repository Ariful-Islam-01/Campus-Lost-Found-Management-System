...
function hasUserClaimedItem($userId, $itemId)
{
    $db = getDBConnection();
    $stmt = $db->prepare("SELECT COUNT(*) FROM claims WHERE user_id = :user_id AND item_id = :item_id");
    $stmt->execute(['user_id' => $userId, 'item_id' => $itemId]);
    return $stmt->fetchColumn() > 0;
}

function submitClaim($userId, $itemId)
{
    $db = getDBConnection();
    $stmt = $db->prepare("INSERT IGNORE INTO claims (user_id, item_id) VALUES (:user_id, :item_id)");
